 ...
 * 抽象服务端面板示例
 */
public abstract class AbstractGUIServer extends AbstractServer {
    ...
    protected AbstractGUIServer(String serverName) {
        super(serverName);

        //设置 Logger，主体为 logPanel
        logger.setLogPane(logPane);
        loadLogPanel();
    }
    ...
    protected Component loadCommonModePanel() {
        ...
        //普通更新规则编辑器
        openCommonModeRuleEditor.addActionListener(new RuleEditorActionListener(commonMode, commonModeList, (GUIServerInterface) getServerInterface(), logger));
    ...
    protected Component loadOnceModePanel() {
        ...
        //补全更新规则编辑器
        openOnceModeRuleEditor.addActionListener(new RuleEditorActionListener(onceMode, onceModeList, (GUIServerInterface) getServerInterface(), logger));
