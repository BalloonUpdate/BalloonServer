 ...
 * @param fileThreadPool 文件 Hash 计算线程池
 * @param hashAlgorithm Hash 算法类型
 * @param completedBytes 进度（可空）
 * @param completedFiles 已完成的文件数量（可空）
 */
public record DirInfoTask(File directory, ExecutorService fileThreadPool, String hashAlgorithm,
                          AtomicLong completedBytes, AtomicInteger completedFiles)
        ...
        for (File file : fileList) {
            if (file.isFile()) {
                ...
                FutureTask<SimpleDirectoryObject> dirCounterTask = new FutureTask<>(
                        new DirInfoTask(file, fileThreadPool, hashAlgorithm, completedBytes, completedFiles));
